...
const iconMapping: Record<QuizMode, { label: string; icon: any }> = {
  "Multiple Choice": { label: "MC", icon: CheckSquare },
  "True/False": { label: "TF", icon: Layers },
  "Memory Card": { label: "FC", icon: Copy },
  "Fill in Blank": { label: "FB", icon: LayoutGrid },
};
      ...
      <DropdownMenu>
        <DropdownMenuTrigger className="flex items-center gap-2 outline-none group">
          <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-white text-xs font-black rotate-12 group-hover:rotate-0 transition-transform">
            {iconMapping[selectedMode]?.label || "QZ"}
          </div>
          <div className="flex items-center">
            <span className="font-bold text-xs">{selectedMode}</span>
        ...
        >
          ...
          <DropdownMenuItem
            onClick={() => handleSelectMode("True/False")}
            className="flex items-center gap-3 p-3 rounded-xl cursor-pointer"
          >
